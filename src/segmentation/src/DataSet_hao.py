...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.resize(image, (640, 320), interpolation=cv2.INTER_NEAREST)
        label = cv2.imread(label_name, 0)
        label = cv2.resize(label, (640, 320), interpolation=cv2.INTER_NEAREST)

        #if 'hao' in image_name:
        #    if 0 in np.unique(label):
        #        label[label==0] = 255                    
        #    if 38 in np.unique(label):
        #        label[label==38] = 0
