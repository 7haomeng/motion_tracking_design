...
class LoadData:
    ...
    def readFile(self, fileName, trainStg=False):
        ...
        with open(self.data_dir + '/' + fileName, 'r') as textFile:
            for line in textFile:
                ...
                #print(line_arr[0].strip())
                ## my pre-process

                #print('..........')
                #if 'hao' in img_file:
                #    if 0 in np.unique(label_img):
                #        label_img[label_img==0] = 255                    
                #    if 38 in np.unique(label_img):
                #        label_img[label_img==38] = 0
                #    print('extract hao file')
    ...
    def processData(self):
        ...
        return_val = self.readFile('train_hao.txt', True)

        print('Processing validation data')
        return_val1 = self.readFile('val_hao.txt')
