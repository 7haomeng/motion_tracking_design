...
void point_data_csv(const std_msgs::Float64MultiArray::ConstPtr& point_msg){
	//std::ofstream myfile;
	point_myfile.open ("/home/graduationv2/DynamicTracking-LKOpticalFlow/src/LKOpticalFlow/csv/point_data.txt", std::ios_base::app);
...
void tracked_point_data_csv(const std_msgs::Float64MultiArray::ConstPtr& point_msg){
	//std::ofstream myfile;
	point_myfile.open ("/home/hao/motion_tracking_design/src/LKOpticalFlow/csv/tracked_point_data.txt", std::ios_base::app);
	//myfile << ",origin_point.x,origin_point.y,origin_point.z,target_point.x,target_point.y,target_point.z\n";
	//myfile << "Data no." << i << ",";
	for(int k = 0; k < point_msg->data.size(); k++){
		if(!isnan(point_msg->data[k])){
...
int main( int argc, char* argv[] ){
	...
	ros::Subscriber tracked_point_sub = nh.subscribe("lk/tracked_point_data", 1000, tracked_point_data_csv);
